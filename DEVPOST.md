...
```
A script supervisor stands just off set with a clipboard, and between every take writes down where the mug sat, whether the jacket was buttoned, which hand held the phone. It is the one job on a film shoot still done entirely on paper, and when a line gets missed nobody finds out until the edit. By then it is a pickup day: a published survey of US production markets (https://giggster.com/guide/reports/film-shoot-costs-major-us-cities-2026/) prices a fixed 6-person, 10-hour shoot day at $1,440 in the cheapest market (Santa Fe, NM) and $3,020 in the most expensive (Boston, MA).
...
The rolling view is the same instrument in its operating mode. Pick a reference take, press Roll, and every few seconds the current camera frame is checked against what that reference recorded. When something contradicts it, the verdict is spoken into the wearer's ear through the glasses' own speakers, because a script supervisor is watching the scene, not a screen. Finding the mug on the wrong side at wrap documents a reshoot. Finding it twenty seconds into the take prevents one. Median verdict is 4.4 seconds on the deployed service, and every one of those checks is written back to ClickHouse with the take it was checking, so "which take did the rolling check flag most" is a query the agent can answer afterwards. Every model call the system makes, whether an extraction, a rolling check or a question, is traced into the same ClickHouse cluster as a row in agent_runs with its model, latency, outcome and any fallback, so the agent can be asked about its own behaviour the same way it is asked about the shoot. The dashboards and data tickets a script supervisor would otherwise need are replaced by a question.

The question box is open on a public URL and lets a model compose SQL against a live cluster, so it has three layers and each was tested by attacking it: the model refuses destructive intent (asked to ignore its instructions and drop a table, it declined and attempted no destructive SQL at all), the MCP server's write and drop flags are set off explicitly rather than left to a library default, and the credential the MCP subprocess connects with is a cluster user granted SELECT and nothing else, which returns ClickHouse code 497 for DROP, INSERT, TRUNCATE, CREATE TABLE and ALTER while SELECT is unaffected. The last layer is the one that matters, because it holds even if the first two fail, and /api/capabilities reports "sql_credential": "readonly" from the same function that builds the subprocess environment, so you can check it rather than take my word for it. Worth saying plainly: that layer was missing for part of the build while this paragraph already claimed it. The service was running as the admin user, and I found out by aiming a DROP TABLE at what I believed was a readonly credential and watching the observations table disappear. It came back from the JSON state in two minutes, because ClickHouse here is a projection of files on disk rather than the only copy, and there is now a test that fails if the admin credential can reach the SQL agent again.

What it costs: 1.6 cents per take, measured on real footage through the same Vertex AI path the deployment runs, at Google's published rates. A 40-take day is about 63 cents, or 0.04% of the day it is guarding. Neither number is mine. Google meters the tokens and sets the price (https://ai.google.dev/gemini-api/docs/pricing); the industry sets the day rate (https://giggster.com/guide/reports/film-shoot-costs-major-us-cities-2026/). Both are shown on screen in the demo video. What is deliberately not claimed is a count of reshoots prevented, because that would be my own code grading my own homework.
