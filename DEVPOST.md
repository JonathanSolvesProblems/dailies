...
The question box is open on a public URL and lets a model compose SQL against a live cluster, so it has three layers and each was tested by attacking it: the model refuses destructive intent (asked to ignore its instructions and drop a table, it declined and attempted no destructive SQL at all), the MCP server's write and drop flags are set off explicitly rather than left to a library default, and the ClickHouse credential is itself readonly, which returns code 164 for DDL, INSERT, TRUNCATE and DROP driven straight through the server while SELECT is unaffected. The last layer is the one that matters, because it holds even if the first two fail.
